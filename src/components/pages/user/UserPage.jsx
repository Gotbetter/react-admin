...
export default function UserPage() {
    ...
    const { mutate: deleteAUser } = useMutation(({userId}) => deleteUser(userId), {
        staleTime: Infinity,
        onError: (err) => {
            ...
            <GraphTemplate columns={columns} paddings={paddings}>
                {users.map(user => (
                    <List key={user.user_id} grid={columns.length}>
                        ...
                        <UserInfo padding={paddings[3]}>{user.updated_date[0]+"/"+user.updated_date[1]+"/"+user.updated_date[2]}</UserInfo>
                        <UserInfo padding={paddings[4]}>
                            {user.role_type === "USER" && user.user_id !== loginUser.user_id ? (
                                <Btn color={YELLOW} >{"수정"}</Btn>
                            ) : (
                                <Btn color={GREY} cursor="true">{"수정"}</Btn>
                            )}
                        </UserInfo>
                        <UserInfo padding={paddings[5]}>
                            {user.role_type === "USER" && user.user_id !== loginUser.user_id ? (
                                <Btn color={PURPLE} onClick={() => deleteAUser({userId: user.user_id})}>{"삭제"}</Btn>
                            ) : (
                                <Btn color={GREY} cursor="true">{"삭제"}</Btn>
                            )}
                        </UserInfo>
                    </List>
                ))}
...
const Btn = styled.button`
    width: 54px;
    height: 24px;
    background-color: ${props => (props.color ? `${props.color}` : '#9747FF')};
    color: white;
    text-align: center;
    font-size: 11px;
    font-style: normal;
